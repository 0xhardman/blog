...
interface IconCloudProps {
  ...
  images?: string[]
  onIconHover?: (iconIndex: number | null) => void
}
...
export function IconCloud({ icons, images, onIconHover }: IconCloudProps) {
    ...
    if (rect) {
      ...
      if (!isDragging) {
        ...
        iconPositions.forEach((icon) => {
          ...
          if (dx * dx + dy * dy < radius * radius) {
            foundHoveredIcon = icon.id
          }
        })

        if (foundHoveredIcon !== hoveredIconIndex) {
          setHoveredIconIndex(foundHoveredIcon)
          onIconHover?.(foundHoveredIcon)
        }
      }
    }
  ...
  useEffect(() => {
    ...
    const animate = () => {
      ...
      iconPositions.forEach((icon, index) => {
        ...
        // Add glow effect for hovered icon
        if (isHovered) {
        ...
        if (icons || images) {
          ...
          ctx.fillStyle = isHovered ? "#3b82f6" : "#4444ff"
          ctx.fill()
          ctx.fillStyle = "white"
